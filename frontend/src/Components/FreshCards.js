    ...
    <>
      <div className="grid grid-cols-2 xs:grid-cols-2  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {(iswishlist?(wishlist?.length):(allItems?.length)) > 0 && !load
          ? (iswishlist?(wishlist):(allItems)).slice(0, cardCount).map((item, i) => (
              <div className="cursor-pointer" key={item.id || i}>
                {load ? (
                  <Card isLoading={true} />
                ) : (
                  <Card isLoading={false} item={item}  iswishlist={iswishlist} />
                )}
              </div>
            ))
          : load &&
      ...
      {(iswishlist?(wishlist?.length):(allItems?.length)) === 0 && !load && (
        <div className="flex w-full flex-col items-center justify-center text-center p-6">
          {
            iswishlist?<FaHeartBroken className="text-gray-400 text-6xl mb-4 animate-pulse" />:<FaSearch className="text-gray-400 text-6xl mb-4 animate-pulse" />
          }
          <h2 className="text-2xl font-semibold text-gray-700">
            {iswishlist?"💔 Oops! Your wishlist is feeling a little lonely.":"No results found! 😞"}
          </h2>
          <p className="text-gray-500 mt-2">
            {iswishlist?"Your wishlist is empty. Start adding items you love! ❤️":"Try a different search, adjust the filters, or check your spelling."}
          ...
          >
            {iswishlist?
            <>
              <FaSearch className="text-lg" />Discover
            </>:<>
